...
Usage :
 ...
 > python kangaroo.py -p 02CEB6CBBCDBDF5EF7150682150F4CE2C6F4807B349827DCDBDD1F2EFA885A2630 -keyspace 800000000000000000000000000000:FFFFFFFFFFFFFFFFFFFFFFFFFFFFFF -rand -ncore 2
...
if platform.system().lower().startswith('win'):
    ...
    logging.error('[-] Unsupported Platform currently for ctypes dll method. Only [Windows and Linux] is working')
    sys.exit()
...
###############################################################################
logging.info('[+] Starting CPU Kangaroo.... Please Wait     Version [%s]', parser.version)
...
###############################################################################
if flag_random1:
    logging.info('[+] Search Mode: Random Start then Continuous Range Search from it')
elif flag_random:
    logging.info('[+] Search Mode: Random Start after every Range 0XFFFFFFFFFFFFFF key search')
else:
    logging.info('[+] Search Mode: Range search Continuous in the given range')
...
while True:
    logging.info('\r[+] Scanning Range %s : %s', hex(range_st), hex(range_en))
    pvk_found = run_cpu_kangaroo(range_st, range_en, dp, ncore, mx, upub)
    with open('scannedkeys.txt', 'a') as ab:
        ab.write(pvk_found.hex() + '\n')
    if int(pvk_found.hex(),16) != 0: 
        logging.info('\n============== KEYFOUND ==============')
        logging.info('Kangaroo FOUND PrivateKey : 0x%s', pvk_found.hex())
        logging.info('======================================')
